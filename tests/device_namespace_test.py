...
int main() {
 ...
 assert((opened==std::vector<std::string>{"/dev/coyote_fpga_7_v3", "/dev/coyote_fpga_7_reconfig", "/dev/coyote_fpga_7_reconfig"}));
 assert((mutexes==std::vector<std::string>{"mutex_dev_7_vfpa_3", "reconfig_mtx"}));
 std::string longest(64,'x');
 setenv("COYOTE_DEVICE_PREFIX",longest.c_str(),1);
 assert(coyote::cDeviceNamespace().regionPath(0,0)=="/dev/"+longest+"_0_v0");
 ...
 assert(snapshot.reconfigurationPath(0)=="/dev/coyote_versal_fpga_0_reconfig");
 assert(snapshot.regionMutex(0,0)=="coyote_versal_fpga_mutex_dev_0_vfpa_0");
 assert(snapshot.reconfigurationMutex()=="coyote_versal_fpga_reconfig_mtx");
 fails([&]{ snapshot.regionPath(0,-1); });
 fails([&]{ snapshot.regionMutex(0,-1); });
 { coyote::cRcnfg r(7); }
 for (const auto &name : removed) assert(name=="reconfig_mtx");
