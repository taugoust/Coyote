...
// Caller holds pid_lock and mmu_lock, excluding every mapping operation.
static void retire_context(struct vfpga_dev *device, int ctid) {
    unsigned long flags;
    spin_lock_irqsave(&device->irq_lock, flags);
    device->context_generation[ctid]++;
    device->pid_array[ctid] = 0;
    spin_unlock_irqrestore(&device->irq_lock, flags);
    put_pid(device->context_pid[ctid]);
    device->context_pid[ctid] = NULL;
    device->num_free_ctid_chunks++;
}
    ...
    if(ref_cnt == 0) {
        ...
        hash_for_each_safe(hpid_ctid_map[device->id], bkt, tmp_h_node, tmp_h_entry, entry) {
            list_for_each_safe(l_p, l_n, &tmp_h_entry->ctid_list) {
                ...
                retire_context(device, l_entry->ctid);
    ...
    switch (command) {
        ...
        case IOCTL_REGISTER_CTID:
            ...
            if (ret_val != 0) {
                ...
                struct pid *context_pid = find_get_pid(hpid);
                struct task_struct *owner = get_pid_task(context_pid, PIDTYPE_PID);
                if (!owner) {
                    put_pid(context_pid);
                    mutex_unlock(&device->pid_lock);
                    return -ESRCH;
                ...
                device->context_pid[ctid] = context_pid;
                unsigned long flags;
                spin_lock_irqsave(&device->irq_lock, flags);
                device->context_generation[ctid]++;
                device->pid_array[ctid] = hpid;
                spin_unlock_irqrestore(&device->irq_lock, flags);
