 ...
 *   - Framing mode (no UFC/NFC; finite RX FIFO fails closed on overflow)
...
module aurora_module (
    ...
    // FIFO resets remain tied to their clock-domain reset signals. Channel
    // status is reported separately and does not reset buffered traffic.
    wire user_rstn = ~reset_pb_sync_uclk;

    // Aurora RX is push-only. Detect the precise cycle in which the CDC FIFO
    // cannot accept a presented beat and latch a fail-closed transport fault.
    // A link reset is required before traffic may be admitted again; silently
    // dropping a beat would allow a truncated record to join a later epoch.
    wire rx_fifo_ready;
    logic rx_overflow_uclk;
    always_ff @(posedge user_clk_out) begin
        if (!user_rstn)
            rx_overflow_uclk <= 1'b0;
        else if (rx_tvalid && !rx_fifo_ready)
            rx_overflow_uclk <= 1'b1;
    end

    xpm_cdc_single #(.DEST_SYNC_FF(2), .INIT_SYNC_FF(0), .SIM_ASSERT_CHK(0), .SRC_INPUT_REG(0))
        u_rx_overflow_sync (.src_clk(user_clk_out), .src_in(rx_overflow_uclk),
    ...
    // the aclk-side consumer to keep up. FIFO refusal is latched above and
    // withdraws the peer endpoint; it must never be treated as a valid stream
    // with a silently missing beat. The always-ready loopback remains within
    // the finite FIFO contract.
    axis_data_fifo_aurora_rx inst_rx_cdc (
        .s_axis_aclk    (user_clk_out),
        .s_axis_aresetn (user_rstn),
        .s_axis_tvalid  (rx_tvalid),
        .s_axis_tready  (rx_fifo_ready),   // observed for fail-closed overflow detection
