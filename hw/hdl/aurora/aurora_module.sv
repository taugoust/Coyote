 ...
 *   - Framing mode with Immediate Native Flow Control (NFC)
 ...
 *     reverse on the 256-bit Aurora boundary so external nets stay
 *     little-endian [N-1:0] (Coyote convention).
 *   - Packs/unpacks 256-bit Aurora transfers in user_clk_out before/after
 *     512-bit asynchronous FIFOs. The 250 MHz shell side can therefore drain
 *     line rate without a 256-bit clock-crossing bottleneck.
 *
...
module aurora_module (
    ...
    // User AXI4-Stream — 512-bit, aclk domain
    AXI4S.m                     m_aurora_rx,   // received data -> shell
    AXI4S.s                     s_aurora_tx,   // shell -> transmit
    ...
    logic                  pma_init;
    logic                  reset_pb;
    logic                  transport_fault_init;

    always_ff @(posedge init_clk) begin
        if (sys_reset || transport_fault_init) begin
            init_cnt <= '0;
            pma_init <= 1'b1;
            reset_pb <= 1'b1;
        end else begin
    ...
    aurora_loopback_ip inst_aurora (
        ...
        // Native flow control generated from RX FIFO occupancy
        .s_axi_nfc_tdata           (nfc_tdata),
        .s_axi_nfc_tvalid          (nfc_tvalid),
        .s_axi_nfc_tready          (nfc_tready),
        // GT serial pins
        .rxp                       (gt_rxp_in),
        .rxn                       (gt_rxn_in),
        .txp                       (gt_txp_out),
    ...
    wire tx_fifo_aresetn = aresetn && channel_up && !rx_overflow &&
                           !hard_err && !mmcm_not_locked && gt_pll_lock;

    // ---------------------------------------------------------------------
    // AXI4-Stream CDC: both FIFOs cross 512-bit complete shell beats. Width
    // conversion runs at user_clk_out, where two 256-bit Aurora transfers fit
    // within one 512-bit FIFO write and the 250 MHz destination can drain the
    // approximately 103 Gb/s wire rate.
    // ---------------------------------------------------------------------
    wire [511:0] tx_fifo_data;
    wire [63:0]  tx_fifo_keep;
    wire         tx_fifo_last;
    wire         tx_fifo_valid;
    wire         tx_fifo_ready;
    ...
    axis_data_fifo_aurora_tx inst_tx_cdc (
        ...
        .m_axis_tkeep   (tx_fifo_keep),
        .m_axis_tlast   (tx_fifo_last)
    );

    aurora_tx_512_to_256 inst_tx_width_adapter (
        .aclk      (user_clk_out),
        .aresetn   (user_rstn && channel_up_w),
        .s_tdata   (tx_fifo_data),
        .s_tkeep   (tx_fifo_keep),
    ...
    wire [511:0] rx_packed_data;
    wire [63:0]  rx_packed_keep;
    wire         rx_packed_last;
    wire         rx_packed_valid;
    wire         rx_fifo_ready;
    ...
    aurora_rx_256_to_512 inst_rx_width_adapter (
        ...
        .s_tlast   (rx_tlast),
        .s_tvalid  (rx_tvalid),
        .m_tdata   (rx_packed_data),
        .m_tkeep   (rx_packed_keep),
        .m_tlast   (rx_packed_last),
        .m_tvalid  (rx_packed_valid),
        .m_tready  (rx_fifo_ready),
        .overflow  (rx_pack_overflow)
    );

    axis_data_fifo_aurora_rx inst_rx_cdc (
        .s_axis_aclk    (user_clk_out),
        .s_axis_aresetn (user_rstn),
        .s_axis_tvalid  (rx_packed_valid),
        .s_axis_tready  (rx_fifo_ready),
        .s_axis_tdata   (rx_packed_data),
    ...
    // the resulting link-down/up interval is the transport epoch boundary.
    logic rx_overflow_uclk;
    wire transport_fault_uclk = rx_overflow_uclk ||
        (channel_up_w && (hard_err_w || mmcm_not_locked_w || !gt_pll_lock_w));
    always_ff @(posedge user_clk_out) begin
        if (!user_rstn)
            rx_overflow_uclk <= 1'b0;
        else if (rx_pack_overflow)
            rx_overflow_uclk <= 1'b1;
    end

    xpm_cdc_single #(.DEST_SYNC_FF(2), .INIT_SYNC_FF(0), .SIM_ASSERT_CHK(0), .SRC_INPUT_REG(0))
        u_transport_fault_init_sync (
            .src_clk(user_clk_out), .src_in(transport_fault_uclk),
            .dest_clk(init_clk), .dest_out(transport_fault_init));
    xpm_cdc_single #(.DEST_SYNC_FF(2), .INIT_SYNC_FF(0), .SIM_ASSERT_CHK(0), .SRC_INPUT_REG(0))
        u_rx_overflow_sync (
            .src_clk(user_clk_out), .src_in(rx_overflow_uclk),
            .dest_clk(aclk), .dest_out(rx_overflow));
