 ...
 *   - SupportLevel=0  (shared logic / QPLL inside the core)
 *   - drp_mode=Native (no AXI4-Lite DRP slave on the wrapper in Vivado 2023.2)
 *
 * The IP wrapper:
...
module aurora_module (
    ...
    //
    // Port naming convention (PG074, drp_mode=Native, SupportLevel=0):
    ...
    //   - drp_clk_in: required input even when drp_mode=Native (clocks the
    //     init/reset logic). Tied to init_clk.
    //   - QPLL ports: at SupportLevel=0, the core is self-contained — no QPLL
    //     fan-in ports. The wrapper exports gt_qpllclk_quad2_out and
    //     gt_qpllrefclk_quad2_out for downstream cores; we leave them open.
    // ---------------------------------------------------------------------
